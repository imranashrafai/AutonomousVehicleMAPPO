...
def find_all_checkpoints():
    """Find all available model checkpoints - FIXED to find all runs."""
    checkpoints = {
        'MAPPO': [],
        'RMAPPO': []
    }
    ...
    for algo in ['mappo', 'rmappo']:
        ...
        if algo_path.exists():
            # Get ALL run folders, not just limited ones
            run_folders = sorted(algo_path.glob("run*"), key=lambda x: x.name)
            
            for run_folder in run_folders:
                model_path = run_folder / "models"
                # Check if the models directory exists and has actor.pt
...
def evaluate_model_sync(checkpoint_path, algo_name, render=True, num_agents=4):
    """
    Evaluate model using subprocess (synchronous) - FIXED render flag.
    """
    cmd = [
        PYTHON_EXE,
        str(TRAIN_SCRIPT),
    ...
    # Only add render flag if render is True
    if render:
...
def run_simulation_and_capture(map_code, num_agents, traffic_density, traffic_mode,
    ...
    # Configure the environment (kept same, safe)
    env_config = {
        "map": map_code,
    ...
    try:
        ...
        # Precompute snapshot step numbers
        ...
        while step < steps:
            # ---------- FASTER ACTIONS HERE ----------
            # Base action from env
            base_action = env.action_space.sample()

            # If action is continuous (common case), force straight + full throttle
            if isinstance(base_action, np.ndarray) and base_action.shape[0] >= 2:
                base_action = base_action.astype(np.float32)
                # Index 0 = steering, 1 = throttle/brake in MetaDrive
                base_action[0] = 0.0   # no steering (straight)
                base_action[1] = 1.0   # full throttle (max acceleration)

            # Same fast action for all controllable agents
            actions = {
                agent_id: (base_action.copy() if isinstance(base_action, np.ndarray) else base_action)
            ...
            # Handle dict frames (MetaDrive sometimes returns dict of cameras)
            if isinstance(frame, dict):
                frame = frame.get("main_camera", list(frame.values())[0] if frame else None)

            # Save frame into video buffer
...
def main():
    ...
    with tab1:
        ...
        with eval_col3:
            render_eval = st.checkbox("Enable Rendering", value=False, help="Uncheck to run evaluation without visualization (faster)")
            ...
            if selected_checkpoint:
                ...
                with col_btn2:
                    if st.button("▶️ START EVALUATION", type="primary", use_container_width=True):
                        with st.spinner(f"Evaluating {algo_choice} - {run_choice}..."):
                            ...
                            if render_eval:
                                progress_placeholder.info("🔄 Starting evaluation with rendering enabled...")
                            else:
                                progress_placeholder.info("🔄 Starting evaluation without rendering (headless mode)...")
                            ...
                            if result['success']:
                                ...
                                - Try running with rendering disabled for faster execution
                                """)
    ...
    with tab2:
        ...
        if view_option == "📈 Algorithm Comparison (MAPPO vs RMAPPO)":
            ...
            if st.button("🔄 Load Algorithm Metrics", type="primary"):
                load_individual_algorithm_metrics(algo_choice)
    ...
    with tab3:
        ...
        # Sidebar Configuration for Simulation
        st.sidebar.markdown("---")
            ...
            if st.button("🚀 RUN SIMULATION & CAPTURE", type="primary", use_container_width=True):
                if not PANDA3D_AVAILABLE:
                    ...
                    try:
                        ...
                        status_placeholder.success(f"✅ Simulation complete! Saved {len(saved_files)} images to `{out_dir}`")
                        
                        if saved_files:
                            st.markdown("### 📸 Captured Snapshots")
                            
                            # Display all captured images
                            for img_path in saved_files:
                                img = Image.open(img_path)
                                st.image(img, caption=f"📷 {Path(img_path).name}", use_container_width=True)
